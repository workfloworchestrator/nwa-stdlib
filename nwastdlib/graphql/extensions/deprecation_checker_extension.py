...
def get_field_deprecation(info: GraphQLResolveInfo) -> Optional[str]:
...
class DeprecationCheckerExtension(SchemaExtension):
    """Graphql extension that logs the use of deprecated fields and paths."""
...
def make_deprecation_checker_extension(
    query: Optional[type] = None, mutation: Optional[type] = None
) -> type[DeprecationCheckerExtension]:
    def deprecations_for(_type: Optional[type]) -> DeprecatedPaths:
        type_def: Optional[TypeDefinition] = getattr(_type, "_type_definition", None) if _type else None
        if not type_def:
            return {}
